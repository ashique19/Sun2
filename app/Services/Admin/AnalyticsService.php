    ...
    public function yearOverview(int $year): array
    {
        ...
        $cogsPerOrder = DB::table('order_products')
            ...
            ->selectRaw("COALESCE(SUM(
                CASE
                    WHEN COALESCE(cogs_orders.is_replacement, 0) = 1 AND COALESCE(order_products.price, 0) <= 0 THEN 0
                    ELSE {$qty} * COALESCE(order_products.unit_cost, order_products.purchase_price, 0)
                END
            ), 0) as cogs")
