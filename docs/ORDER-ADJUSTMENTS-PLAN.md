...
Status: **implemented on branch `cursor/implement-money-reseller-plans-dd17`** (schema, services, admin/storefront wiring, multi-coupon checkout, payment ledger, courier_charge sync). Remaining: full multi-line admin adjustment editor (dual-write from scalars still), per-courier fee field mapping, rollout/ops commands, nice-to-haves.
...
- [x] Confirm percent base: remaining merchandise after prior discount/coupon lines — **locked: yes** (implemented in CouponStackingService)
- [x] Confirm `min_order` check: against original subtotal — **locked: original subtotal** (implemented in CouponStackingService)
- [x] Confirm max stack count — **locked: unlimited** (no soft cap in v1)
- [x] Confirm whether admin may stack coupons that checkout would reject — **locked: yes, admin override with logged note** (adminOverride flag in CouponStackingService)
- [x] Confirm cancel/return behavior for `used_count` — **locked: v1 does NOT decrement used_count on cancel**
- [x] Confirm whether charges can be negative — **locked: no** (enforced in OrderAdjustmentSync::validateLine)
- [x] Confirm rounding — **locked: integer taka rounding at persist for admin; 2-decimal internally**
- [x] Confirm admin freeform discounts also respect product max caps — **locked: yes, with audited override**
- [x] Confirm default for existing products: `max_discount = null` (uncapped) — **locked: null = uncapped until staff set values**
- [x] Confirm cancel/return policy for `courier_charge` — **locked: keep last fee** (no automatic zero/clear)
- [ ] Per-courier: which API/webhook fields carry the actual fee (Steadfast / Pathao / Redx / CarryBee) — pending courier integration
- [x] Confirm `collected_amount` vs `paid_amount` — **locked: paid = all successful txns; collected = COD subset** (implemented in OrderPaymentSync)
- [x] Confirm overpayment allowed — **locked: allow with due=0** (OrderPaymentSync sets due=max(0,...))
- [x] Confirm cancel with prior gateway advance — **locked: v1 leave paid, flag due; refund flow deferred**
- [x] Confirm webhook no fee — **locked: leave courier_charge unchanged** (OrderCourierChargeSync only writes on change)

### B. Schema & migrations

- [x] Migration: create `order_adjustments` — `database/migrations/2026_07_23_110000_add_order_money_adjustments.php`
- [x] Migration: create `order_adjustment_logs` (with `field` + `phase` for delivery/courier cost too)
- [x] Migration: add `orders.courier_charge` decimal(12,2) default 0
- [x] Migration: enhance `payment_transactions` (`kind`, `paid_at`, `payment_method_id`, `external_id`)
- [x] Seed `payment_methods` (`cod`, `bkash`, `nagad`, `cash`, `bank`) — insertOrIgnore in migration
- [x] Migration: add `products.max_discount` (nullable decimal)
- [x] Migration: add `order_products.max_discount` snapshot (nullable decimal)
- [x] Unique index: `(order_id, coupon_id)` — MySQL NULLs are distinct so non-coupon lines (coupon_id=NULL) are not blocked
- [x] Eloquent: `OrderAdjustment`, `OrderAdjustmentLog`, `PaymentTransaction`, `PaymentMethod`; update `Product`, `OrderProduct`, `Order`
- [x] `Order` relations: `adjustments()`, `adjustmentLogs()`, `paymentTransactions()`, keep `coupon()` for compat
- [ ] Factories for tests — deferred (UI phase)
- [x] Backfill in migration: charge/coupon/discount lines from scalars + log rows written
- [ ] Verify backfill on a copy of production (40MB data dump when available)

### C. Domain services

- [x] `OrderTotalCalculator` — `app/Services/Orders/OrderTotalCalculator.php`; returns `OrderTotals` DTO with total, cogs, netRevenue, deliveryMargin, charges, discounts
- [x] `Order::netRevenue()` / `Order::cogs()` / `Order::deliveryMargin()` helpers — added to `app/Models/Order.php`
- [x] `OrderAdjustmentSync` — `app/Services/Orders/OrderAdjustmentSync.php`; replaceAdjustments + materializeFromScalars
- [x] `OrderAdjustmentAuditor` — `app/Services/Orders/OrderAdjustmentAuditor.php`; log + logField helpers
- [x] `OrderCourierChargeSync` — `app/Services/Orders/OrderCourierChargeSync.php`; set() writes only on change, never touches delivery_charge
- [ ] `OrderDeliveryChargeAudit` — deferred to UI phase
- [x] `OrderPaymentRecorder` — `app/Services/Orders/OrderPaymentRecorder.php`; record() creates completed txn + syncs
- [x] `OrderPaymentSync` — `app/Services/Orders/OrderPaymentSync.php`; sole writer of paid/due/status/cod_amount
- [ ] Wire dispatch / webhook / tracking / deliver / cancel paths to `OrderCourierChargeSync` — deferred (courier integration phase)
- [ ] Rewire deliver / webhook to record payment txn then sync — deferred (courier integration phase)
- [ ] Provider fee parsers (Steadfast, Pathao, Redx, CarryBee) — deferred (courier integration phase)
- [x] `CouponStackingService` — `app/Services/Orders/CouponStackingService.php`; validate, resolve, buildAdjustmentLine, remainingSubtotal
- [x] `ProductDiscountCap` — `app/Services/Orders/ProductDiscountCap.php`; orderCouponCap + allocateAcrossLines (proportional, per-line clamp)
- [ ] Stop diverging storefront vs admin formulas — deferred (UI phase; calculator is ready)
- [x] `delivery_charge` stays independent of adjustment sync and `courier_charge` — enforced by design
- [ ] Admin order edit: prefer lines as source of truth — deferred (UI phase; materializeFromScalars available)
- [x] Legacy path materialize helper — `OrderAdjustmentSync::materializeFromScalars`
- [x] `Order::collectableAmount()` uses residual cod_amount / due_amount — unchanged, correct by design
