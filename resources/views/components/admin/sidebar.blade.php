    ...
    <nav class="p-4 space-y-1 text-sm">
        ...
        @unless ($isModeratorOnly)
            ...
            <a href="{{ route('admin.coupons') }}" wire:navigate
                class="block rounded-lg px-3 py-2 {{ request()->routeIs('admin.coupons*') ? 'bg-[#FAF6EF] font-semibold text-[#C9A227]' : 'text-[#6B6459] hover:bg-[#FAF6EF]' }}">
                Coupons
            </a>
            <a href="{{ route('admin.hero-slides') }}" wire:navigate
                class="block rounded-lg px-3 py-2 {{ request()->routeIs('admin.hero-slides*') ? 'bg-[#FAF6EF] font-semibold text-[#C9A227]' : 'text-[#6B6459] hover:bg-[#FAF6EF]' }}">
                Hero Slides
            </a>
            ...
            <div class="space-y-1 pt-2">
                <p class="px-3 pt-1 pb-0.5 text-xs font-semibold uppercase tracking-wide text-[#8C8474]">Users</p>
                <div class="ml-3 space-y-0.5 border-l border-[#E7DFCF] pl-2">
                    <a href="{{ route('admin.users.customers') }}" wire:navigate
                        class="block rounded-lg px-3 py-1.5 {{ request()->routeIs('admin.users.customers') ? 'bg-[#FAF6EF] font-semibold text-[#C9A227]' : 'text-[#6B6459] hover:bg-[#FAF6EF]' }}">
                        Customers
                    </a>
                    <a href="{{ route('admin.users.moderators') }}" wire:navigate
                        class="block rounded-lg px-3 py-1.5 {{ request()->routeIs('admin.users.moderators') ? 'bg-[#FAF6EF] font-semibold text-[#C9A227]' : 'text-[#6B6459] hover:bg-[#FAF6EF]' }}">
                        Moderators
                    </a>
                </div>
            </div>
