...
<div>
    ...
    @if ($steadfastExpectedApiBalance !== null)
        <a href="{{ route('admin.couriers') }}"
            ...
            <p class="min-w-0 truncate text-xs text-[#8C8474]" title="Book receivable from courier (collected − courier charge − COD % − withdrawals)">
        ...
        <div class="mb-6 rounded-xl border border-sky-200 bg-white overflow-hidden">
            ...
            <div class="divide-y divide-[#EFE7D6]">
                @foreach ($returnHubArrivals as $order)
                    <div wire:key="return-hub-arrival-{{ $order->id }}" class="space-y-2.5 px-4 py-3 hover:bg-[#FAF6EF]/50">
                        <div class="flex flex-wrap items-center gap-2">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            <button type="button"
                                ...
                                wire:target="markReturnHubReceived({{ $order->id }})"
                                ...
                                <span wire:loading.remove wire:target="markReturnHubReceived({{ $order->id }})">Mark as received</span>
                                <span wire:loading wire:target="markReturnHubReceived({{ $order->id }})">Saving…</span>
                            </button>
                            <a href="{{ route('admin.orders.return-pending') }}"
    ...
    @if (($steadfastWebhookInbox ?? collect())->isNotEmpty())
        <div class="mb-6 rounded-xl border border-[#E0D6C2] bg-white overflow-hidden">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-[#EFE7D6] px-4 py-3">
                <div class="min-w-0">
                    <h2 class="text-sm font-semibold text-[#1E1E1E]">Latest Steadfast webhooks</h2>
            ...
            <div class="divide-y divide-[#EFE7D6]">
                @foreach ($steadfastWebhookInbox as $entry)
                    @php
                        ...
                        $summary = $steadfastWebhookSummaries[$entry->id] ?? 'webhook update';
