<div @class([
    'relative inline-flex shrink-0' => $status && ($status['valid'] ?? false),
])>
    @if ($status && ! $status['valid'])
        @teleport('#inbox-fb-token-banner')
    ...
    @elseif ($status && $status['valid'])
        <div wire:key="fb-token-gate-valid">
            <button type="button"
                wire:click="$toggle('showReplace')"
                class="inline-flex h-10 w-10 items-center justify-center rounded-full border border-emerald-200 bg-emerald-50 text-emerald-700 hover:border-emerald-300 hover:bg-emerald-100"
            ...
            @if ($showReplace)
                <div class="absolute right-0 top-full z-50 mt-2 w-[min(20rem,calc(100vw-2rem))] rounded-xl border border-emerald-200 bg-emerald-50 p-3 shadow-lg">
                    <input type="password"
                        wire:model="tokenInput"
                        autocomplete="off"
