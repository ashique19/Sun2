                                        ...
                                        <div>
                                            ...
                                            <div class="flex min-h-[280px] items-center justify-center bg-[#FAF6EF] px-4 py-4">
                                                <template x-if="savedPreviewUrl">
                                                    {{-- Outer stage keeps padding so resize handles stay tappable; overlays must be positioned against the image frame only (same 0–1 space as canvas bake). --}}
                                                    <div class="inline-block max-h-[46vh] max-w-full touch-none p-8 sm:p-4" data-text-overlay-stage>
                                                        <div class="relative inline-block max-h-[46vh] max-w-full" data-overlay-image-frame>
            ...
            @if ($showPricedImageModal)
                @php
                    ...
                    $pricedStampConfig = [
                        ...
                        'hasPricedImage' => filled($product?->priced_image_path),
                    ];
                @endphp
                <div class="fixed inset-0 z-[80] flex items-stretch justify-center bg-black/50 sm:items-center sm:p-4"
                    ...
                    <div class="flex h-dvh w-full max-w-4xl flex-col overflow-hidden bg-white shadow-xl sm:h-auto sm:max-h-[min(90dvh,42rem)] sm:rounded-xl"
                        ...
                        {{-- One scrollable body so Save/Delete stay reachable on short viewports. --}}
                        <div class="min-h-0 flex-1 overflow-y-auto overscroll-contain" data-priced-image-modal-scroll>
                            {{-- Keep Alpine stamp editor out of Livewire morphs so drag/resize state is not destroyed mid-sync. --}}
                            <div wire:ignore data-priced-stamp-editor-host>
                                <div x-data="pricedImageStampEditor(@js($pricedStampConfig))"
                                    data-priced-stamp-editor>
                                    <div class="space-y-3 border-b border-[#EFE7D6] px-4 py-3">
                                        <div class="flex gap-1.5" role="group" aria-label="Text position">
                                            @foreach ([
                                                'top-left' => 'Top left',
                                                'top-right' => 'Top right',
                            ...
                            <div class="space-y-3 border-t border-[#EFE7D6] px-4 py-4">
                                @error('pricedImage')
