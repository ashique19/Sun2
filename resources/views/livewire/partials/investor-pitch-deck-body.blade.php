...
@endphp
    ...
    <div class="mb-5 flex flex-wrap items-center gap-2" role="group" aria-label="Report year">
        <span class="mr-1 text-xs font-medium text-[#5C564C]">Year</span>
        @foreach ($years as $y)
    ...
    <section class="mb-6 overflow-hidden rounded-2xl border border-[#EFE7D6] bg-gradient-to-br from-[#FAF6EF] via-white to-[#F3EBD8] p-6 sm:p-8">
        <p class="text-xs font-semibold tracking-[0.2em] text-[#7A6114] uppercase">Sundoritoma</p>
        ...
        <p class="mt-3 max-w-xl text-sm text-[#6B6459]">
            ...
            @if ($deck['is_partial_year'])
                <span class="ml-1 rounded-full bg-white/80 px-2 py-0.5 text-[11px] text-[#7A6114] ring-1 ring-[#EFE7D6]">YTD</span>
            @endif
        </p>

        <div class="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <div class="rounded-xl bg-white/80 p-4 ring-1 ring-[#EFE7D6]">
                <p class="text-xs text-[#5C564C]">Placed GMV</p>
            ...
            <div class="rounded-xl bg-white/80 p-4 ring-1 ring-[#EFE7D6]">
                <p class="text-xs text-[#5C564C]">Orders</p>
            ...
            <div class="rounded-xl bg-white/80 p-4 ring-1 ring-[#EFE7D6]">
                <p class="text-xs text-[#5C564C]">Collected (delivered)</p>
            ...
            <div class="rounded-xl bg-white/80 p-4 ring-1 ring-[#EFE7D6]">
                <p class="text-xs text-[#5C564C]">Return rate</p>
    ...
    <section class="mb-6 rounded-2xl border border-[#EFE7D6] bg-white p-6">
        ...
        <div class="mt-5 overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-[#EFE7D6] text-left text-xs text-[#5C564C]">
                ...
                <tbody class="tabular-nums">
                    @foreach ([
                        ...
                        <tr class="border-b border-[#F5F0E6]">
                            ...
                            <td class="py-2.5 pr-4 text-[#5C564C]">{{ $pri }}</td>
    ...
    <section class="mb-6 rounded-2xl border border-[#EFE7D6] bg-white p-6">
        ...
        <p class="mt-1 text-sm text-[#5C564C]">Delivered in {{ $deck['window']['label'] }} · contribution before ads &amp; salaries</p>
        <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-xl bg-[#FAF6EF] p-4">
                <p class="text-xs text-[#5C564C]">Merch GM (known COGS)</p>
            ...
            <div class="rounded-xl bg-[#FAF6EF] p-4">
                <p class="text-xs text-[#5C564C]">Merch GP (est.)</p>
            ...
            <div class="rounded-xl bg-[#FAF6EF] p-4">
                <p class="text-xs text-[#5C564C]">Delivery margin</p>
            ...
            <div class="rounded-xl bg-[#FAF6EF] p-4">
                <p class="text-xs text-[#5C564C]">Contribution (est.)</p>
    ...
    <section class="mb-6 rounded-2xl border border-[#EFE7D6] bg-white p-6">
        <div class="flex flex-wrap items-end justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-[#5C564C]">Placed GMV by month · {{ $deck['year'] }} vs {{ $deck['prior_year'] }}</p>
            </div>
            <div class="flex items-center gap-3 text-[11px] text-[#6B6459]">
                <span class="inline-flex items-center gap-1.5"><span class="h-2 w-2 rounded-full bg-[#8F7218]"></span>{{ $deck['year'] }}</span>
                ...
                <div class="grid grid-cols-[2.5rem_1fr_auto] items-center gap-3 text-xs">
                    ...
                    <div class="space-y-1">
                        <div class="h-2 overflow-hidden rounded-full bg-[#F5F0E6]">
                            <div class="h-full rounded-full bg-[#8F7218]"
                    ...
                    <span class="min-w-[8.5rem] text-right tabular-nums text-[#1E1E1E]">
                        {{ $fmt($row['gmv']) }}
                        <span class="text-[#5C564C]">/ {{ $fmt($row['prior_gmv']) }}</span>
                    </span>
                </div>
            @endforeach
        </div>
    ...
    <div class="mb-6 grid gap-6 lg:grid-cols-3">
                ...
                @forelse ($deck['channels'] as $row)
                    <li class="flex items-center justify-between gap-3">
                        ...
                        <span class="tabular-nums text-[#1E1E1E]">
                            ...
                            <span class="text-[#5C564C]">· {{ $row['orders'] }}</span>
                        </span>
                    </li>
                @empty
                    <li class="text-[#5C564C]">No orders in window.</li>
                @endforelse
            </ul>
        </section>
            ...
            <ul class="mt-4 space-y-3 text-sm">
                @forelse ($deck['geos'] as $row)
                    <li class="flex items-center justify-between gap-3">
                        ...
                        <span class="tabular-nums text-[#1E1E1E]">
                            {{ $row['orders'] }}
                            <span class="text-[#5C564C]">· {{ $fmt($row['gmv']) }}</span>
                        </span>
                    </li>
                @empty
                    <li class="text-[#5C564C]">No orders in window.</li>
        ...
        <section class="rounded-2xl border border-[#EFE7D6] bg-white p-6">
            ...
            <p class="text-xs text-[#5C564C]">Delivered line revenue</p>
            <ul class="mt-4 space-y-3 text-sm">
                ...
                @empty
                    <li class="text-[#5C564C]">No delivered lines.</li>
                @endforelse
            </ul>
        </section>
    </div>
    ...
    <section class="mb-6 rounded-2xl border border-[#EFE7D6] bg-white p-6">
        ...
        <div class="mt-4 grid gap-4 sm:grid-cols-3">
            <div class="rounded-xl border border-[#EFE7D6] p-4">
                <p class="text-xs text-[#5C564C]">Unsettled dispatched</p>
            ...
            <div class="rounded-xl border border-[#EFE7D6] p-4">
                <p class="text-xs text-[#5C564C]">Collection on delivered</p>
            ...
            <div class="rounded-xl border border-[#EFE7D6] p-4">
                <p class="text-xs text-[#5C564C]">Returns</p>
    ...
    <section class="mb-2 rounded-2xl border border-dashed border-[#E0D6C2] bg-[#FAF6EF]/60 p-6">
        ...
        @if ($showAdminLinks ?? false)
            <div class="mt-4 flex flex-wrap gap-3 text-xs">
                <a href="{{ route('admin.analytics.pnl') }}" wire:navigate class="font-medium text-[#7A6114] hover:underline">Open P&amp;L →</a>
                <a href="{{ route('admin.analytics.ordered-delivered') }}" wire:navigate class="font-medium text-[#7A6114] hover:underline">Ordered vs delivered →</a>
                <a href="{{ route('admin.analytics.category-revenue') }}" wire:navigate class="font-medium text-[#7A6114] hover:underline">By category →</a>
            </div>
        @endif
    </section>
