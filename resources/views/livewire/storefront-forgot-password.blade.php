<x-storefront.shell>
    <div class="mx-auto max-w-md px-4 py-10">
        ...
        <p class="text-sm text-[#5C564C] text-center mb-8">{{ __('storefront.forgot_password_subtitle') }}</p>
        ...
        @if ($step === 'request')
            <form wire:submit="sendReset" wire:key="forgot-password-request" class="rounded-xl border border-[#EFE7D6] bg-white p-6 space-y-4">
                <div>
                    ...
                    <p class="text-xs text-[#5C564C] mt-2">{{ __('storefront.email_mobile_hint') }}</p>
                </div>
                <button type="submit"
            ...
            <form wire:submit="resetWithOtp" wire:key="forgot-password-otp" class="rounded-xl border border-[#EFE7D6] bg-white p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" wire:model="otp" wire:key="forgot-password-otp-input" maxlength="6" inputmode="numeric"
                        class="w-full tracking-[0.4em] text-center text-lg rounded-lg border border-[#E0D6C2] px-4 py-3 focus:border-[#8F7218] focus:outline-none focus:ring-1 focus:ring-[#8F7218]">
                    @error('otp') <p class="text-xs text-rose-600 mt-1">{{ $message }}</p> @enderror
                    @if (app()->hasDebugModeEnabled())
                        <p class="text-xs text-[#5C564C] mt-1">Debug: OTP <strong>123456</strong></p>
                    @endif
                </div>
                <div>
        ...
        <p class="text-sm text-center text-[#6B6459] mt-6">
            <a href="{{ route('login') }}" wire:navigate class="text-[#7A6114] hover:underline">{{ __('storefront.back_to_login') }}</a>
        </p>
    </div>
</x-storefront.shell>
