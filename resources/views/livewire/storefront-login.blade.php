<x-storefront.shell>
    <div class="mx-auto max-w-md px-4 py-10">
        ...
        <p class="text-sm text-[#5C564C] text-center mb-8">{{ __('storefront.login_subtitle') }}</p>
        ...
        <form wire:submit="login" class="rounded-xl border border-[#EFE7D6] bg-white p-6 space-y-4">
            ...
            <div class="flex items-center justify-between text-sm">
                <label class="flex items-center gap-2 text-[#6B6459]">
                    <input type="checkbox" wire:model="remember" class="rounded border-[#E0D6C2] text-[#7A6114] focus:ring-[#8F7218]">
                    {{ __('storefront.remember_me') }}
                </label>
                <a href="{{ route('password.request') }}" wire:navigate class="text-[#7A6114] hover:underline">{{ __('storefront.forgot_password') }}</a>
            </div>
            <button type="submit"
        ...
        <p class="text-sm text-center text-[#6B6459] mt-6">
            ...
            <a href="{{ route('register') }}" wire:navigate class="text-[#7A6114] hover:underline">{{ __('storefront.create_an_account') }}</a>
        </p>
    </div>
</x-storefront.shell>
