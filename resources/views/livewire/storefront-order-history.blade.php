<x-storefront.shell>
    <div class="mx-auto max-w-6xl px-4 py-8">
        <h1 class="font-serif text-3xl font-semibold mb-8">Order History</h1>
                ...
                <div class="rounded-xl border border-[#EFE7D6] bg-white overflow-hidden">
                    @if ($orders->isEmpty())
                        <div class="p-6 text-sm text-[#6B6459]">
                            No orders yet.
                            <a href="{{ route('home') }}" wire:navigate class="text-[#C9A227] hover:underline">Browse products</a>
                        </div>
                    @else
                            ...
                            @foreach ($orders as $order)
                                <a href="{{ route('account.orders.show', $order) }}" wire:navigate
                                    ...
                                    <div class="min-w-0">
                                        ...
                                        <div class="text-[#8C8474]">{{ $order->placed_at?->format('d M Y, h:i A') }} &middot; {{ $order->items_count }} item(s)</div>
                                    </div>
