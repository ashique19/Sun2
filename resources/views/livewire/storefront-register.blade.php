<x-storefront.shell>
    <div class="mx-auto max-w-md px-4 py-10">
        ...
        <p class="text-sm text-[#5C564C] text-center mb-8">{{ __('storefront.join_tagline') }}</p>
        ...
        <form wire:submit="register" class="rounded-xl border border-[#EFE7D6] bg-white p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium mb-1">{{ __('storefront.email_label') }} <span class="font-normal text-[#5C564C]">({{ __('storefront.optional') }})</span></label>
        ...
        <p class="text-sm text-center text-[#6B6459] mt-6">
            ...
            <a href="{{ route('login') }}" wire:navigate class="text-[#7A6114] hover:underline">{{ __('storefront.log_in_link') }}</a>
        </p>
    </div>
</x-storefront.shell>
