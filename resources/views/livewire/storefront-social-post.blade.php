<x-storefront.shell>
    ...
    <div class="mx-auto max-w-6xl px-4 py-10">
        <div class="mb-6">
            ...
            <p class="mt-1 text-xs text-[#5C564C]">Hand-picked products · Published on {{ $post->created_at->toFormattedDateString() }}</p>
        </div>
                ...
                @if ($facebookPub && $facebookPub->external_url)
                    <div class="mt-4">
                        <a href="{{ $facebookPub->external_url }}" target="_blank" rel="noreferrer"
                            class="inline-flex items-center rounded-full bg-[#8F7218] px-5 py-2 text-sm font-semibold text-white hover:bg-[#7A6114]">
                            View on Facebook
                        </a>
                    </div>
                @endif
        ...
        <div class="mt-10">
            ...
            @if ($post->products->isEmpty())
                <div class="text-sm text-[#5C564C]">No products attached.</div>
            @else
                    ...
                    @foreach ($post->products as $product)
                        ...
                        <a href="{{ route('product.show', $product) }}" wire:navigate
                            ...
                            <div class="aspect-square bg-[#F1EADB] flex items-center justify-center overflow-hidden">
                                ...
                                @else
                                    <span class="text-[#7A6114] text-3xl">&#9670;</span>
                                @endif
                            </div>
            ...
            <div class="mt-10">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <a href="{{ route('category.show', $primaryCategory) }}" wire:navigate
                        class="text-sm font-semibold text-[#7A6114] hover:underline">
